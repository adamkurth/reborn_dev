    ...
    def _make_croman_data(self):
        if self.atomic_nums is None:
            self.form_facts_arr = np.ones(
                (self.Npix + self.Nextra_pix, 1), dtype=np.float32)
        ...
        self.atomIDs = np.array([self.atom_id_lookup[z]
                                 for z in self.atomic_nums])
...
#       combine form factors with q_vectors for faster reads...
        ...
        self.q_buff = clcore.ClCore.to_device_static(
            q_zeros, dtype=np.float32, queue=self.queue)
