...
% Griffiths "script r"
\newcommand{\rcurs}{{\mbox{$\resizebox{.09in}{.08in}{\includegraphics[trim= 1em 0 14em 0,clip]{fonts/ScriptR}}$}}}
\newcommand{\brcurs}{{\mbox{$\resizebox{.09in}{.08in}{\includegraphics[trim= 1em 0 14em 0,clip]{fonts/BoldR}}$}}}
...
The electromagnetic fields from a moving point 
...
appendix \ref{appendix:inhomogeneous} for details), which yields the retarded 
potentials:
\begin{align}
...
potentials (see appendix \ref{appendix:lienard} for details)
\begin{align}
V(\vec{r}, t) &= \frac{1}{4\pi\epsilon_0}    
\frac{q}{\left| \rcurs - \brcurs \cdot \vec{\beta} \right|} &
\vec{A}(\vec{r}, t) &= \frac{\mu_0}{4\pi}    
\frac{q\vec{v}}{\left| \rcurs - \brcurs \cdot \vec{\beta} \right|} \;,
\end{align}
where $\vec{\beta} = \vec{v}/c = \dot{\vec{r}}'(t_r)/c$. From the 
Li\'enard-Wiechert potentials, Griffiths used equation
...
[(c^2 - v^2)\vec{u} + \brcurs \times (\vec{u} \times \vec{a})] \; , 
\quad\quad \vec{B}(\vec{r}, t)
...
E_{\text{rad}}(\vec r, t) \;.
\end{align}
The radiation field in equation \ref{eqn:Erad} dominates because it falls off in 
proportion to $1/\rcurs$ whereas the generalized
Coulomb field falls off in proportion to $1/\rcurs^2$.  
We therefore focus on the radiation field
\begin{align}
...
Consider a point charge that oscillates about the origin with position equal to
the real part of $\vec{r}' 
= r_0 \exp(-i\omega t) \hvec{r}'$.
In addition to the far-field approximation, we make the dipole radiation 
assumptions: 
\begin{enumerate}
 \item The dipole is small compared to 
the distance to our observation point, which means that $r \gg r'$ and hence 
$\brcurs \approx \vec{r}$.
\item The charged particle is non-relativistic, which means that $v \ll c$, and 
hence $\vec{u} \approx c \hrcurs$.  This assumption also implies that 
$r_0\omega/c = 2\pi r_0/\lambda \ll 1 $; the wavelength is much
larger than the dipole, and the time it takes light to traverse the extent of 
the dipole is small compared to the period of oscillation.
\end{enumerate}
Plugging these in, we get the dipole fields
...
\end{align}
The power radiated through an area $d\vec{a} = r^2 
...
\end{align}
The total power radiated into all angles is is
\begin{align}
P = \int \frac{dP}{d\Omega} d\Omega =  \frac{\mu_0 }{16\pi^2 c}  \int |\hvec{r} \times    \ddot{\vec{p}}|^2 d\Omega  \; .
\end{align}
Choosing coordinates such that $|\hvec{r} \times \hvec{p}|^2 = \sin^2\theta$ 
yields a factor of $8\pi/3$.  The final result is the Larmor formula:
\begin{align}
P =  \frac{\mu_0 \ddot{p}^2}{6\pi c}  \; .
\end{align}
For sinusoidal oscillations, a time average of the above introduces a factor 
of $1/2$:
\begin{align}
\avg{\frac{dP}{\Delta \Omega}} &= \frac{\mu_0 \omega^4 p^2}{32\pi^2 c} |\hvec{r}\times\hvec{p}|^2  \\
\langle P \rangle &=  \frac{\mu_0 \omega^4 p^2}{12\pi c} \; . \label{eqn:avgpwer}
\end{align}


\section{Thompson scatter from a single free electron}

When a free electron exposed to an electromagnetic wave, the electron 
accelerates and radiates, which we refer to as \emph{Thompson scattering}.  
The force on the electron 
(mass $m_e$ and charge $e$) due to an incident light beam with oscillating 
electric field $\vec{E}_0(t)$ results in the acceleration
\begin{align}
...
\;.
\end{align}
We therefore have the power captured by a small detector pixel:
...
vector becomes (see appendix \ref{sec:poynt} for details)
\begin{align}
...
which gives the expression for the measured diffraction signal
% \footnote{The 
...
\begin{align} \label{eqn:Pfactor}
 \mathcal{P}(\vec{q}) &= 1 - \alpha(\hvec{k} \cdot \hvec{E}_1 )^2 - 
(1-\alpha)(\hvec{k} \cdot \hvec{E}_2 )^2 \;.
\end{align}
The special cases of linear polarized ($\alpha=1$) and unpolarized light light 
($\alpha=1/2$) often appear in textbooks as
\begin{align}
 \mathcal{P}(\phi) &= 1 - \sin^2\phi \;, & \alpha &=1   \\
 \mathcal{P}(\theta) &= \frac{1}{2}\left[ 1 + \cos^22\theta \right] \;, &\alpha &=\frac{1}{2} 
\end{align}
where $2\theta$ is the scattering angle.  The use of dot products rather than 
angles in equation \ref{eqn:Pfactor} speeds up calculations and avoids the need
to figure out what $\phi$ is.

\section{Weak Diffraction (Born approximation)}

Starting with the scattering field from a single electron, we build up the 
diffraction from many electrons by superposition of the individual dipole 
fields.  The diffraction pattern is caused by the phase shifts that result from
electrons that are not located at the origin.
For an electron at position 
$\vec{r}'$ under the influence of a plane wave
\begin{align}
...
\end{align}
Defining the outgoing wavevector $\vec{k} = \omega/c\; \hvec{r}=2\pi/\lambda \hvec{r}$ 
(directed at the detector) and $\vec{q} = \vec{k}-\vec{k}_0$, we get
\begin{align}
...
\end{align}
This form factor is just the Fourier transform of the electron density, with the conjugate variable $\vec{q}$ being defined by the incoming and outgoing wavevectors.  The time-averaged Poynting vector is
\begin{align}
\avg{\vec{S}(\vec{r})} =  r_e^2 
...
Griffiths chapter 9 by adding a restoring force and a damping force in 
...
frequency-dependent features of diffraction.  
% The correction to be made to our 
...
as (appendix \ref{sec:3d1d} derives \ref{eqn:form})
\begin{align}
...
\exp(-i\omega t) \;.
\end{align}
Using the usual definitions for a non-magnetic linear material, 
