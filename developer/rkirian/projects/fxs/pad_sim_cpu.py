...
if not os.path.exists(conf["pdb_file"]):
    conf["pdb_file"] = crystal.get_pdb_file(
        conf["pdb_file"], save_path=".", use_cache=False
    )
...
for i in range(len(conf["pad_geometry_file"])):
    p = detector.load_pad_geometry_list(conf["pad_geometry_file"][i]).binned(
        conf["pad_binning"][i]
    )
    p.center_at_origin()
...
rho = dmap.place_atoms_in_map(
    x, f, mode="nearest"
)  # FIXME: replace 'nearest' with Cromer Mann densities
f_dens_water = atoms.xraylib_scattering_density(
    "H2O", water_density, conf["photon_energy"], approximate=True
)
if conf["solvent_contrast"]:
    ...
    rho[rho != 0] -= (
        f_dens_water * dmap.voxel_volume
    )  # FIXME: Need a better model for solvent envelope
# rho = ifftshift(rho)
...
rho, side = denss.create_density_map(
    ...
    cell=cell,
    create_bio_assembly=conf["create_bio_assembly"],
    map_resolution=conf["map_resolution"],
    map_oversampling=conf["map_oversample"],
)
if view_density_projections:
...
protein_number_density = (
    conf["protein_concentration"] / cryst.molecule.get_molecular_weight()
)
n_proteins_per_drop = int(
    protein_number_density * 4 / 3 * np.pi * conf["drop_radius"] ** 3
)
...
f2p = (
    ...
    * geom.solid_angles()
    * geom.polarization_factors(beam=beam)
)
...
if 0:
    ...
    if conf["one_particle"]:
        nppd = 1
    if conf["droplet"]:
        p_vecs = placer.particles_in_a_sphere(
            sphere_diameter=dd, n_particles=nppd, particle_diameter=protein_diameter
        )
    else:
        p_vecs = placer.particles_in_a_cylinder(
            cylinder_diameter=dd,
            cylinder_length=dd,
            n_particles=nppd,
...
class PADGetter(FrameGetter):
    ...
    def get_data(self, frame_number=0):
        ...
        dd = (
            conf["drop_radius"] * 2
            + (np.random.rand() - 0.5) * conf["drop_radius_fwhm"]
        )
        if conf["droplet"]:
            vol = 4 / 3 * np.pi * (dd / 2) ** 3
        else:
        ...
        df = dataframe.DataFrame(
            pad_geometry=geom, beam=beam, raw_data=intensities, mask=mask
        )
        return df
...
q_vecs = (
    ...
    * np.pi
    / lam
    * np.vstack(
        [st * np.cos(phi), st * np.sin(phi), (1 - np.cos(theta)) * np.ones(n_phi)]
    ).T.copy()
)
...
for i in range(n_shots):
    ...
    if conf["gas_background"]:
        print(
            "No gas background set up for FXS sims because of polarization factor handling"
        )
        intensities += gasbak
    I_ring = intensities
    ...
    if doprint:
        print(
            f"{nppd} particles, {np.sum(I_ring)} photons in ring ({np.sum(I_ring)/n_phi} per pixel)"
        )
...
acf_sum_noisy += np.mean(acf_sum[a:m]) - np.mean(
    acf_sum_noisy[a:m]
