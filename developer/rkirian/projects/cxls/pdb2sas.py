...
def pdb2sas(pdb, q=np.linspace(0, 0.5, 501), return_pr=True, photon_energy=10000, dispersion_correction=True):
    ...
    (i.e. no solvent considerations).
    pdb - a saxstats PDB file object.
    q - q values to use for calculations. Default 0.0 < q < 0.5, with 501 data points.
    """
    ...
    ff = np.zeros((natoms, nsh), dtype=np.complex)
    
    for i in range(natoms):
        ff[i, :] = formfactor(pdb.atomtype[i], q=qsh, photon_energy=photon_energy, dispersion_correction=True)
    if numba:
    ...
    if return_pr:
        nr = 1001
        Pr = np.zeros((nr, 2))
        r = np.linspace(0, D, nr)
        P = Ish2P(Ish=Ish, D=D, r=r)
        r, Pfilt = filter_P(r, P, qmax=Iq[:, 0].max()*0.9)
        Pr[:, 0] = r
        Pr[:, 1] = Pfilt
        return Iq, Pr
    else:
...
def Ish2P(Ish, D, r=None, dr=None):
...
def formfactor(element, q=(np.arange(500)+1)/1000., photon_energy=10000, dispersion_correction=True):
    ...
    if dispersion_correction:
        Z = atoms.atomic_symbols_to_numbers(element)
        ff = ff.copy() + atoms.henke_dispersion_corrections(atomic_numbers=Z, photon_energies=photon_energy*1.6022e-19)
    return ff
