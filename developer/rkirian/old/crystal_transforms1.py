...
# Scatter plot of finite lattice points
scat = Scatter3D()
...
# Diffraction intensity simulation
...
max_h = np.max(np.abs(h_vecs), axis=0)
clcore = ClCore()
amps = clcore.phase_factor_qrf(h_vecs, lat_x_coords)
intensity = pad.reshape(np.abs(amps)**2)
pg.image(intensity)
keep_open()
