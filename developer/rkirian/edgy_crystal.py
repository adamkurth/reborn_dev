...
for c in range(1):
    ...
    width = 5 + np.random.rand(1)*2
    length = 15 + np.random.rand(1)*2
    for i in range(spacegroup.n_molecules):
        lat = lats[i]
        com = mol_x_coms[i, :]
    ...
    if viewcrystal:
        ...
        for i in range(spacegroup.n_molecules):
            ...
            scat.add_points(unitcell.x2r(x_vecs), color=bright_colors(i, alpha=0.5), size=5)
            sym_vecs = spacegroup.apply_symmetry_operation(i, au_x_vecs)
            scat.add_points(unitcell.x2r(sym_vecs), color=bright_colors(i, alpha=0.01), size=1)
    ...
    for i in range(spacegroup.n_molecules):
        ...
        r = unitcell.x2r(x)
        clcore.phase_factor_qrf(q_rot_gpu, lats[1].occupied_r_coordinates, a=amps_lat_gpu, add=False)
