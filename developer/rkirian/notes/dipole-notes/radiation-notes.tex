...
Consider a point charge that oscillates with position $\vec{r}' = r_0 \cos(-i\omega t) \hvec{r}'$.%, where 
it is understood that we take the real part to get the observable quantity.
%, which gives the velocity $\vec{v} = - r_0 \omega \sin(\omega t) \hvec{r}'$ and acceleration
% $\vec{a} = - r_0 \omega^2 \cos(\omega t) \hvec{r}'$.
...
We further assume that the charged particle is non-relativistic, which means that
%\footnote{Equivalently, $r_0\omega/c  \ll 1 $ or $r_0/\lambda \ll 1$; the wavelength is much
...
% We can identify the dipole moment as $\vec{p}_0(t_r) = q r_0 \cos(\omega t_r) \hvec{r}'$ and
%rewrite the above:
% \begin{align}
% \vec{E}_\text{dip} =  \frac{\mu_0 }{4\pi} \frac{1}{r} [\hvec{r} \times ( \hvec{r} \times  \ddot{\vec{p}}_0(t_r))] \; .
% \end{align}
% If we had many point charges distributed through space that all satisfy the same assumptions
% mentioned above, we simply sum the fields from each of the dipoles to get
% \begin{align}
% \vec{E}_\text{dip} =  \frac{\mu_0 }{4\pi} \frac{1}{r} [\hvec{r} \times ( \hvec{r} \times   \sum_n
% \ddot{\vec{p}}_n(t_r))] \; .
% \end{align}
% We may extend the to the continuum limit and re-write the above as in Griffiths 11.56:
% \begin{align}
% \vec{E}_\text{dip} =  \frac{\mu_0 }{4\pi} \frac{1}{r} [\hvec{r} \times ( \hvec{r} \times
% \ddot{\vec{p}}(t_r)]
% \end{align}
% where the total dipole moment is $\vec{p} = \int \rho(\vec{r}') \vec{r}' d\tau'$.  %By ``localized'', we assume that the timescale over which the field changes is
% much less than $\vec{r}_i'/c$ (we will be more explicit about this in the next section, and you may
% also look to Griffiths equation 11.50 for further elaboration).
% We might as well rewrite the above using the total dipole moment $\vec{p}(t_r) = \sum_n
%\ddot{\vec{p}}_n(t_r)$, which gives us Griffiths equation 11.56:
% \begin{align}
% \vec{E}_\text{dip} =  \frac{\mu_0 }{4\pi} \frac{1}{r} [\hvec{r} \times ( \hvec{r} \times
%\ddot{\vec{p}}(t_r))] \; .
% \end{align}
...
%&=  \frac{1}{\mu_0c} \vec{E}_\text{dip}\times(\hvec{r} \times \vec{E}_\text{dip}) \\
%&= \frac{1}{\mu_0c}\left[ \hvec{r}  (\vec{E}_\text{dip}\cdot \vec{E}_\text{dip})  - \vec{E}_\text{dip} (\hvec{r}\cdot \vec{E}_\text{dip}) \right] \\
...
\end{align}
% We often speak in terms of radiation power per solid angle and write
% \begin{align}
% \frac{dP}{d\Omega} = \frac{\mu_0 }{16\pi^2 c} |\hvec{r} \times    \ddot{\vec{p}}|^2   \; .
% \end{align}
...
\end{align}
% The dipole approximation for an oscillator demands that $r' \ll \omega/c$; the dipole is much smaller than the radiation wavelength.

\section{Thompson scatter from a free electron}

An example of a very simple classical scattering process is a free electron exposed to an electromagnetic wave, which results in acceleration and hence re-radiation or \emph{scattering}.  %Electrons are far less massive than protons, and the radiation intensity scales with acceleration squared, therefore electrons are the primary radiators in classical light scattering processes.  
...
\end{align}
where we simplified the expression by defining the classical electron radius%\footnote{There are other ways to develop the classical electron radius.  It is also the result of equating the rest energy $m_ec^2$ with the energy necessary to assemble a charge $e$ into a sphere of radius $r_e$.}
\begin{align}
...
% In the case of x-rays, the incident frequency is so high that nearly all electrons behave as free electrons, including those bound to atoms.  The above formula gives good quantitative results.  
...
\end{align}
In the continuum limit, with electron density $f(\vec{r})$, we have
\begin{align}
\vec{E}_\text{diff}(\vec{r},t) =  r_e\frac{e^{ikr}}{r} e^{-i\omega t}[\hvec{r} \times ( \hvec{r} \times  \vec{E}_0 )] \int f(\vec{r}') e^{-i\vec{q}\cdot\vec{r}'} d^3r' \;.
\end{align}
...
\avg{\vec{S}(\vec{r})} &= r_e^2 \frac{1}{r^2} (I_1 | \hvec{r} \times  \hvec{E}_1 |^2 + I_2 | \hvec{r} \times  \hvec{E}_2 |^2) \left| F(\vec{q}) \right|^2 \hvec{r} \;. %\\
 %&= r_e^2 \frac{1}{r^2} (I_1 | \hvec{r} \times  \hvec{E}_1 |^2 + I_2 | \hvec{r} \times  (\hvec{k}_0\times\hvec{E}_1) |^2) \left| F(\vec{q}) \right|^2 \hvec{r}\;.
\end{align}
...
% The two most common polarizations are (1) linear polarization in which $\alpha=1$ and the polarization factor is often written as 
% \begin{align}
% P_\text{linear}(\vec{q}) = 1 - \cos^2\theta
% \end{align}
% with $\theta$ being the angle between $\hvec{k}$ and $\hvec{E}_1$.  The second
% case is (2) unpolarized light in which case $\alpha = 0.5$.  If we take $\hvec{k}_0=\hvec{z}$ we may write
% \begin{align}
%  P(\vec{q}) = \frac{1}{2} \left( | \hvec{r} \times  \hvec{x} |^2 + | \hvec{r} \times  \hvec{y} |^2\right) \;.
% \end{align}
% Since unpolarized light is symmetric about the $\hvec{k}_0$ axis we may choose $\hvec{r} = \sin\phi \hvec{x} + \cos\phi \hvec{z}$
% and write
% \begin{align}
%  P(\vec{q}) %&= \frac{1}{2} \left( | (\sin\phi \hvec{x} + \cos\phi \hvec{z}) \times  \hvec{x} |^2 + | (\sin\phi \hvec{x} + \cos\phi \hvec{z})\times  \hvec{y} |^2\right) \\
%  &= \frac{1}{2} \left( 1+ \cos^2\phi \right)\;,
% \end{align}
% where $\phi$ is the angle between $\vec{k}_0$ and $\vec{k}$.  Although the polarizations are nicer to look at when expressed as 
% trigonometric functions, the formula in equation \ref{eqn:goodpol} is most convenient when using a  computer and the relation $\abs{\hvec{r}\times\hvec{E}}^2 = 1 - (\hvec{k}\cdot\hvec{E})^2$.


% In a diffraction measurement, it is usually straightforward to extract $\left| f(\vec{q}) \right|^2$ -- the magnitude of the Fourier transform of the electron density, or, if you wish, the wavevector amplitudes of the momentum-space representation of the object.  This equation gives quantitatively correct results in the case of x-rays striking small targets, because x-rays have very high frequency and the electrons, even if bound to atoms/molecules, behave approximately as a plasma that scatters weakly.  This scattering equation has been used to solve the vast majority of molecular structures that are presently known.  Many Nobel prizes have been awarded to scientists who have found creative ways to determine the electron density $\rho(\vec{r})$ given only the measured Fourier magnitudes $|F(\vec{q})|$.  It is often referred to as the ``crystallographic phase problem''.
% 
% It is noteworthy that the solution to the Schr\"odinger equation, under the assumption of weak scattering, also yields the modulus square of a Fourier transform, however, in this case it is the electrostatic potential rather than the electron density that appears in this Fourier transform.  The weak scattering approximation is also known as the first-order Born approximation.
