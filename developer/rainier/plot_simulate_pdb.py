...
plt.imshow(np.log(pad.reshape(np.abs(a_dev.get()) ** 2) + 0.1), interpolation='nearest', cmap='ocean', origin='lower')
plt.title('y: up, x: right, z: beam (towards you)')
plt.show()
...
##########################################################
#rainier edits
a_real_3d=np.abs(np.reshape(a_map_dev.get(),[N,N,N]))
