...
def test_01():
    ...
    f = np.array(
        [[4 + 1j, 3 + 1j, 2 + 1j], [3 + 1j, 2 + 2j, 1 + 3j]], dtype=np.complex128
    )
    ...
    assert (
        np.abs(a[0] - np.real((f[0, 0] + f[1, 0]) * np.conj(f[0, 0] + f[1, 0]))) < 1e-6
    )
    assert (
        np.abs(aa[0] - np.real((f[0, 0] + f[1, 0]) * np.conj(f[0, 0] + f[1, 0]))) < 1e-6
    )
    # Check all others
