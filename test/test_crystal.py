...
def test_02():
    ...
    assert (
        np.max(np.abs(cryst.pdb_dict["scale_matrix"] * 1e10 - cryst.unitcell.o_mat_inv))
        / np.max(np.abs(cryst.unitcell.o_mat_inv))
        < 0.001
    )
...
def test_04():
    ...
    cryst = crystal.CrystalStructure(psi_pdb_file)
    cell = cryst.unitcell
    assert np.max(np.abs(cell.a_vec - np.array([2.81e-08, 0.00e00, 0.00e00]))) < 1e-8
    assert (
        np.max(np.abs(cell.a_mat_inv[0, :] - np.array([2.81e-08, 0.00e00, 0.00e00])))
        < 1e-8
    )
...
def test_05():
    ...
    unitcell = crystal.UnitCell(
        5e-10, 5e-10, 5e-10, 90 * np.pi / 180, 90 * np.pi / 180, 90 * np.pi / 180
    )
...
def test_09():
    ...
    for pdb in ["2W0O", "4BED", "2LYZ"]:
        ...
        assert int(cryst.fractional_coordinates.shape[0] / cryst.n_bio_partners) == len(
            cryst.pdb_dict["atomic_symbols"]
        )
