...
def test_density_map_1():
    ...
    density_map = crystal.CrystalDensityMap(cryst, resolution=30e-10, oversampling=2)

    # Make a density map and populate two of the voxels, rest is all zeros
    num_atoms = len(cryst.x_vecs)
    rho = density_map.place_atoms_in_map(cryst.x_vecs, np.zeros(num_atoms), mode='trilinear')
    rho[3, 3, 3] = 1
    rho[5, 6, 7] = 1

    # Define a DFT function
    def dftn(f):
        q_vec = 2 * np.pi * density_map.h_vecs
        r_vec = density_map.x_vecs

        nx, ny, nz = f.shape
        n = nx * ny * nz

        f = np.ravel(f)
        ft = np.zeros(n, dtype=np.complex64)
        ...
        return ft.reshape((nx, ny, nz))

    # Calculate the intensity via the numpy fft
    intensity_fft = np.abs(np.fft.fftn(rho))**2

    # Calculate the intensity via the dft function above
    intensity_dft = np.abs(dftn(rho))**2
    intensity_dft = np.fft.ifftshift(intensity_dft)
