...
def plugin(self):
    ...
    profiler = detector.RadialProfiler(pad_geometry=pads, beam=self.beam, q_range=(0, np.max(q_mags)), n_bins=n_bins)
    data = profiler.subtract_median_profile(data, mask=mask)
